...
Kindle 端需要越狱并安装 KUAL 才能运行自定义脚本。以下资源可自行参考：

- **[Kindle 越狱指南](https://www.mobileread.com/forums/forumdisplay.php?f=150)** — MobileRead Kindle Developer's Corner，越狱教程和固件选择
- **[KUAL](https://www.mobileread.com/forums/showthread.php?t=203326)** — Kindle Unified Application Launcher，越狱后运行自定义脚本的入口
- **[KOReader](https://github.com/koreader/koreader)** — 开源阅读器，附带 fbink 工具（屏幕刷新用）
- **[MRPI](https://www.mobileread.com/forums/showthread.php?t=251143)** — MobileRead Package Installer，安装 KUAL 扩展的包管理器

### Kindle 端部署

越狱完成后，Kindle 上需要部署 KUAL 扩展脚本，负责从 PC 拉取渲染好的图片并直写 `/dev/fb0`：
...
扩展脚本位于本仓库的 `kual-extensions/kindledesk/`，daemon 位于 `daemon/serve.py`。
...
即时 PUSH 模式还需要在启动 daemon 前设置 `KINDLEDESK_KINDLE_IP`；默认的 PULL 模式不需要该变量。

### 完整安装

按 [KindleDesk 完整安装指南](SETUP.md) 从头操作。它不止列前置条件，而是依次完成：

1. 越狱后的 MRPI + KUAL 安装
2. KOReader / `fbink` 安装与路径检查
3. 把 KUAL 扩展部署到 `/mnt/us/extensions/kindledesk/`
4. 通过 USB 写入 `/mnt/us/kindledesk/pc_ip`
5. 启动并验证 PC daemon
6. 从 KUAL 启动 PULL 显示并检查 `loop.log`
7. 推送第一个测试 widget，确认端到端链路

## 验证与测试

```bash
# 运行项目测试
cd kindledesk-card && python tests/test_push_templates.py -v
