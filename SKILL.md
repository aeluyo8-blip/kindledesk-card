...
description: 为已完成设备安装的 KindleDesk 墨水屏选择、预览和推送 widget，并处理 daemon、PC IP、Kindle IP 与 PULL/PUSH 运行期连接问题。用户提到 Kindle 卡片、墨水屏副屏、待办、专注任务、日程、消息简报、预览、推送或 KindleDesk 运行中断时使用。不负责越狱、KUAL、KOReader 或首次部署。默认只校验/输出 payload，只有用户明确说"推送、发送到 Kindle、现在显示"时才 POST /widget。
metadata:
  trigger_keywords:
    - Kindle 卡片
    - 墨水屏
...
假定用户已经按仓库 `SETUP.md` 完成一次性设备配置。daemon 不可达、推送失败或 Kindle 超过 60 秒未更新时，读取 [运行期连接检查](references/runtime-connection.md)。

只检查 daemon、PC IP、Kindle IP、PULL 日志和 PUSH 模式，不安装或重配越狱、KUAL、KOReader 与 Kindle 扩展。
...
| daemon 不可达 | `KindleDesk daemon unreachable: ...` | 转到 [运行期连接检查](references/runtime-connection.md)，检查 daemon URL 与进程状态 |
...
- "Kindle 怎么越狱？" → 一次性设备安装问题，不属于本 Skill；指向仓库 `SETUP.md`
- "帮我写一篇论文" → 这是写作任务，不是卡片操作
- "今天天气怎么样？" → 这是查询，不是推送；如果用户接着说"推到 Kindle"才触发
- "给我推荐一本书" → 这是推荐，和卡片无关
- "Kindle 屏幕不亮了怎么办" → 普通硬件故障不是本 Skill 的活；仅当 KindleDesk 运行中停止更新时检查 runtime connection
